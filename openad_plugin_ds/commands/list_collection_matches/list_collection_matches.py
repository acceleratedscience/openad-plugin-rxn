...
def list_collection_matches(cmd_pointer, cmd: dict):
    ...
    for c in (pbar := tqdm(collections, total=len(collections), bar_format="{l_bar}{bar}", leave=False)):
        ...
        try:
            # Execute the query
            query = DataQuery(cmd["search_string"], source=[""], limit=0, coordinates=c.source)
    ...
    if not results_table:
        return output_error(plugin_msg("err_no_matching_collections", cmd["search_string"]), return_val=False)

    # Success
    output_success(
        plugin_msg("success_matching_collections", len(results_table), cmd["search_string"]),
        return_val=False,
        pad_top=1,
    )
