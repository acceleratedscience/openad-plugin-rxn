...
def predict_single_reaction(inputs: dict, cmd_pointer):
    ...
    # Define the RXN API
    api = cmd_pointer.login_settings["client"][cmd_pointer.login_settings["toolkits"].index("RXN")]

    # Prepare the data query
    reaction_input = inputs["molecule"]
    val = "val"
    predict_id = None
    ai_model = "2020-08-10"
    ...
    result = rxn_helper.retrieve_result_cache(cmd_pointer, entry_2, "predict_Model-" + ai_model)
    sources = ""
    ...
    try:
        if predict_id is None and ai_model is None:
            predict_reaction_response = api.predict_reaction(reaction_input)
        elif predict_id is None:
            predict_reaction_response = api.predict_reaction(reaction_input, ai_model=ai_model)
        else:
            predict_reaction_response = api.predict_reaction(reaction_input, prediction_id=predict_id)
    ...
    while "response" not in predict_reaction_results:
        try:
            predict_reaction_results = api.get_predict_reaction_results(predict_reaction_response["prediction_id"])
